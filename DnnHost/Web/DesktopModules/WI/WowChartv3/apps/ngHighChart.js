    ...
    return {
        ...
        link: function (scope, element, attrs) {
            ...
            function buildChart() {
                ...
                if (!angular.isUndefinedOrNull(hcOptions.yAxis._plotLines)) {
                    hcOptions.yAxis.plotLines = [];
                    for (var idx = 0; idx < hcOptions.yAxis._plotLines.length; idx++) {
                        var line = hcOptions.yAxis._plotLines[idx];
                        if (line.TypeId == 1) {//fixed value
                            line.Options.value = line.Value;
                        } else if (line.TypeId == 2) { // sql data
                            line.Options.value = settings.PlotLinesData[line.id];
                        ...
                        hcOptions.yAxis.plotLines.push(line.Options);
                    }
                    delete hcOptions.yAxis._plotLines;
                }
